...
import { buildProcessContainmentInsightPrompt } from "@/utils/processContainmentInsightPrompt";
import { generateWithOllama } from "@/utils/ollamaClient";
import { STATUS_RED, STATUS_AMBER } from "@/components/shared/statusColors";

const DEFAULT_MODEL = "llama3.2";

interface ProcessContainmentInsightProps {
...
/**
 * Task 5 — Gen AI. "Process Containment Insight": focused on the Defect x
 * Station process-containment analysis, it reasons about WHY a
 * potential-escape or data-quality-concern signal might be happening,
 * generated fresh from that table each time — nothing here is a
 * pre-written explanation. Named for what it explains rather than
 * "prediction," since it's reasoning about signals already computed from
 * real data, not forecasting something unknown. Deliberately NOT a
 * chatbot: a single "generate" action over one fixed, structured prompt,
 * not an open conversation. Styled distinctly from the deterministic
 * charts (dashed indigo border, "AI-generated" badge) so it's unmistakably
 * a different kind of output — a synthesis, not a measurement.
 */
export function ProcessContainmentInsight({ flaggedItems, allDefects, monthDefects, scope, activeLabel }: ProcessContainmentInsightProps) {
  ...
  async function handleGenerate() {
    const prompt = buildProcessContainmentInsightPrompt(containmentRows, flaggedItems, activeLabel);
    if (!prompt) return;
    setStatus("loading");
    setErrorMessage("");
    ...
    <div className="rounded-xl border-2 border-dashed border-bmw-indigo/40 bg-gradient-to-br from-white to-[#F5F4FF] p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-[14px] font-semibold text-ink">Process Containment Insight</h3>
