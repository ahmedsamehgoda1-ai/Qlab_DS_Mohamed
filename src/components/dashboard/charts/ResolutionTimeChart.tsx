...
import { ZoomOut, ChevronLeft, ChevronRight, Search } from "lucide-react";
...
export function ResolutionTimeChart({
  ...
  // One month at a time — with ~9,000 records spanning a year, plotting
  // everything at once buries the distribution; a month keeps sample size
  // reasonable for the quartile math and keeps the chart legible.
  ...
  const matchesSearch = (p: { defectName: string; station: string }) =>
    !searchLower || p.defectName.toLowerCase().includes(searchLower) || p.station.toLowerCase().includes(searchLower);
      ...
      rightSlot={
        <div className="flex flex-wrap items-center justify-end gap-2">
          <div className="flex items-center gap-1 bg-paper rounded-lg p-0.5">
            ...
            <span className="text-[11px] font-medium text-ink px-1 min-w-[92px] text-center">{monthLabel}</span>
            <button
              onClick={() => goToMonth(1)}
          ...
          >
            ...
            {visibleOutliers.map((o: ResolutionOutlierPoint) => {
              ...
              const hoverLines = [
                ...
                isFlagged ? "Flagged — click to edit comment" : "Click to flag + add a comment",
              ];
              return (
                <g
                  key={o.id}
        ...
        {popover && (
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              >
                {popover.isExisting ? "Save comment" : "Flag defect"}
              </button>
              {popover.isExisting && (
                <button
